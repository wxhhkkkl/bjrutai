...
| 组织人员管理 | 任意深度组织树，环路检测、子树迁移、操作历史、组织停用约束；组织内创建分销员账户、调整归属、设置组织管理员 |
| 组织资质 | 组织级资质上传 (营业执照等)，审核通过后作为组织业务准入门槛 (FR-008)，到期提醒/过期暂停 |
| 推广码 | 组织资质通过后自动生成专属二维码 (含 BJTR 来源标识)，支持刷新和统计 |
| 客户绑定 | 医生录入患者信息 → 调用儒泰接口匹配 → 绑定至分销员，支持解绑/转移 |
| 数据同步 | 每分钟轮询儒泰 getBindUser，自动拉取 getUserBill，游标分页防漏 |
| 贡献值计算 | 实付金额 × 换算系数，按组织树逐级向上汇总 |
| 分账规则 | 按组织层级配置 (固定比例/固定金额/阶梯)，生效时间调度，变更日志 |
| 对账报表 | 多维报表 (绑定/收款/折扣/分配，按组织维度汇总)，Excel 导出，数据偏差 ≤ 0.01% |
| 组织业绩 | 组织管理员在小程序查看授权组织子树业绩汇总与成员贡献值 (本月/累计，不含客户明细) |
