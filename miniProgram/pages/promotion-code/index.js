...
Page({
  ...
  async savePromotionCode() { const url = this.data.profile.qrImage; if (!url) { wx.showToast({ title: '推广二维码暂未生成', icon: 'none' }); return } this.setData({ saving: true }); try { const file = await new Promise((resolve, reject) => wx.downloadFile({ url, success: resolve, fail: reject })); if (file.statusCode !== 200) throw new Error('图片下载失败'); await new Promise((resolve, reject) => wx.saveImageToPhotosAlbum({ filePath: file.tempFilePath, success: resolve, fail: reject })); wx.showToast({ title: '已保存到相册', icon: 'success' }) } catch (error) { const denied = String(error && error.errMsg || '').includes('auth deny'); if (denied) wx.showModal({ title: '需要相册权限', content: '请在设置中允许保存图片到相册后重试', confirmText: '去设置', success: (result) => { if (result.confirm) wx.openSetting({}) } }); else wx.showToast({ title: '保存失败，请重试', icon: 'none' }) } finally { this.setData({ saving: false }) } },
  onShareAppMessage() { return createPromotionShare(this.data.profile) },
  onShareTimeline() { const share = createPromotionShare(this.data.profile); return { title: share.title, query: share.path.split('?')[1] || '', imageUrl: share.imageUrl } }
})
