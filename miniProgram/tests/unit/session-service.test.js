...
test('visitor without a name can browse the home page', () => { assert.equal(getEntry({ userId:'p', role:'personal', profileCompleted:false, activationStatus:'active' }).url, '/pages/home/index') })
test('business member without a profile is routed to profile setup', () => { assert.equal(getEntry({ userId:'p', role:'collaborator', hasBusinessMembership:true, profileCompleted:false, activationStatus:'active' }).url, '/pages/auth/profile-setup/index') })
