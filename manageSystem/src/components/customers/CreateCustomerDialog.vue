<template>
  <el-dialog :model-value="modelValue" title="新建客户" width="480px" :close-on-click-modal="false" @close="reset" @open="init">
    <el-form :model="form" label-width="90px" size="small" ref="formRef" :rules="rules">
      ...
      <el-form-item label="归属客户顾问" prop="distributorId">
        <el-select v-model="form.distributorId" filterable placeholder="选择归属客户顾问" style="width: 100%" :loading="distLoading">
          <el-option v-for="d in distributors" :key="d.distributorId" :value="d.distributorId"
            :label="`${d.name}（${d.phone}）`" />
        </el-select>
        <div v-if="!distLoading && distributors.length === 0" class="hint">该组织下暂无客户顾问，请先在组织人员管理中添加</div>
      </el-form-item>
...
const rules = {
  distributorId: [{ required: true, message: '请选择归属客户顾问', trigger: 'change' }],
