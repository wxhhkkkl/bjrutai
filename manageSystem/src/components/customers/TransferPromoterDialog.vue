<template>
  <el-dialog :model-value="modelValue" title="更改归属客户顾问" width="480px" :close-on-click-modal="false" @close="reset" @open="init">
    <el-form :model="form" label-width="90px" size="small" ref="formRef" :rules="rules">
      <el-form-item label="客户">
        <el-input :model-value="customerName || '-'" disabled />
      </el-form-item>
      <el-form-item label="当前客户顾问">
        <el-input :model-value="currentPromoter || '-'" disabled />
      </el-form-item>
      <el-form-item label="新客户顾问" prop="newDistributorId">
        <el-select v-model="form.newDistributorId" filterable placeholder="选择新客户顾问" style="width: 100%" :loading="distLoading">
          <el-option
        ...
        <div v-if="!distLoading && availableDistributors.length === 0" class="hint">该组织下无其他可用客户顾问</div>
      </el-form-item>
...
const rules = {
  newDistributorId: [{ required: true, message: '请选择新客户顾问', trigger: 'change' }],
...
async function init() {
  ...
  try {
    ...
    ElMessage.error(e.response?.data?.message || '加载客户顾问失败')
  } finally {
...
async function submit() {
  ...
  try {
    ...
    ElMessage.success('归属客户顾问已变更')
    emit('success')
  } catch (e) {
    ElMessage.error(e.response?.data?.message || '变更客户顾问失败')
  } finally {
    saving.value = false
  }
}
